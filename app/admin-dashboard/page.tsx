...
import { getUserProfilePicture, getProfilePictureUrl } from "@/lib/profile-picture"
...
export default function AdminDashboardPage() {
  ...
  const loadProfilePicture = async (userId: string) => {
    try {
      ...
      if (profilePicture) {
        const url = await getProfilePictureUrl(profilePicture.file_path)
  ...
  useEffect(() => {
    const checkUser = async () => {
      ...
      // Fetch user data where auth user ID matches user_id in table
      const { data: userRow, error: userError } = await supabase
        .from("users")
        .select("*")
        .eq("user_id", user.id) // Only get record where user_id matches auth ID
        .single()
      ...
      if (userRow.verified_role !== "admin") {
  ...
  const handleSave = async () => {
    if (!userData || !editedData || !user) return

    setSaving(true)
    try {
      // Only update record where user_id matches current auth user
      const { error } = await supabase
        ...
        .eq("user_id", user.id) // Security: only update own record
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold mb-8 text-center text-black">Admin Dashboard</h1>
      ...
      <Tabs defaultValue="my-listings">
        ...
        <TabsContent value="my-listings">
          <div className="text-center py-12">
            ...
            <p className="text-gray-400 text-sm mt-2">Your saved listings will appear here.</p>
          </div>
        </TabsContent>
        ...
        <TabsContent value="my-info">
          <Card className="rounded-3xl">
            <CardHeader className="flex flex-row items-center justify-between">
              ...
              {!isEditing ? (
                <Button onClick={handleEdit} variant="outline" className="rounded-3xl bg-transparent">
                  <Edit className="h-4 w-4 mr-2" />
                  Edit Profile
                </Button>
              ) : (
                <div className="flex gap-2">
                  <Button onClick={handleCancel} variant="outline" className="rounded-3xl bg-transparent">
                ...
                <div className="space-y-6">
                  <div className="flex items-center space-x-4 mb-6">
                    ...
                    <div>
                      ...
                      <p className="text-gray-500 capitalize">{userData.verified_role}</p>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      {isEditing ? (
                        ...
                        <div className="mt-1 p-2 bg-gray-50 rounded-md">{userData.first_name || "Not provided"}</div>
                      )}
                    </div>
                    <div>
                      ...
                      {isEditing ? (
                        ...
                        <div className="mt-1 p-2 bg-gray-50 rounded-md">{userData.last_name || "Not provided"}</div>
                      )}
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="mt-1 p-2 bg-gray-50 rounded-md capitalize">{userData.verified_role}</div>
                  </div>
                    ...
                    <div>
                      ...
                      {isEditing ? (
                        <div className="flex gap-2 mt-1">
                          <div className="w-32">
                            <Label htmlFor="country_code" className="text-sm text-gray-600">
                              Country Code
                            </Label>
                            <Input
                              id="country_code"
                          ...
                          <div className="flex-1">
                            ...
                            <Input
                              ...
                              onChange={(e) => handleInputChange("phone_local", e.target.value)}
                        ...
                        <div className="mt-1 p-2 bg-gray-50 rounded-md">{formatPhoneDisplay()}</div>
                      )}
                    </div>
                  </div>
